...
import (
	...
	"errors"
	"fmt"
	http2 "github.com/luoxk/restys/http2"
...
// c is never closed.
func (p *Http2ClientConnPool) AddConnIfNeeded(key string, t *Http2Transport, c reqtls.Conn) (used bool, err error) {
	p.mu.Lock()
...
func (c *http2addConnCall) run(t *Http2Transport, key string, tc reqtls.Conn) {
...
var (
	...
	http2errClosedBody         = errors.New("body closed by http.Handler")
	http2errHandlerComplete    = errors.New("http2: request body closed due to http.Handler exiting")
...
type http2ServeConnOpts struct {
	...
	// http.Handler specifies which http.Handler to use for processing
...
type http2serverConn struct {
	...
	wantWriteFrameCh chan http2FrameWriteRequest // from http.Handlers -> serve
	wroteFrameCh     chan http2frameWriteResult  // from writeFrameAsync -> serve, tickles more frame writes
	bodyReadCh       chan http2bodyReadMsg       // from http.Handlers -> serve
	...
	tlsState         *tls.ConnectionState        // shared by all http.Handlers, like net/http
	...
	curHandlers                 uint32 // number of running http.Handler goroutines
...
// http.Handler, this struct intentionally has no pointer to the
...
type http2stream struct {
	...
	flow             http2outflow // limits writing from http.Handler to client
	...
	reqTrailer http.Header // http.Handler's Request.Trailer
}
...
func (sc *http2serverConn) serve() {
	...
	defer close(sc.doneServing) // unblocks http.Handlers trying to send
...
func (sc *http2serverConn) readPreface() error {
	...
	timer := sc.srv.newTimer(http2prefaceTimeout) // TODO: configurable on *http.Server?
	defer timer.Stop()
	select {
	case <-timer.C():
...
// writeDataFromHandler writes DATA response frames from a http.Handler on
// the given stream.
...
func (sc *http2serverConn) writeFrame(wr http2FrameWriteRequest) {
	...
	// does not apply.
	//
	// The serverConn might close an open stream while the stream's http.Handler
	// is still running. For example, the server might close a stream when it
	// receives bad data from the client. If this happens, the http.Handler might
	// attempt to write a frame after the stream has been closed (since the
	// http.Handler hasn't yet been notified of the close). In this case, we simply
	// ignore the frame. The http.Handler will notice that the stream is closed when
...
// Request.Body when the main goroutine panics. Since most http.Handlers read in the
// main ServeHTTP goroutine, this will show up rarely.
var http2errHandlerPanicked = errors.New("http2: http.Handler panicked")
...
func (sc *http2serverConn) wroteFrame(res http2frameWriteResult) {
	...
	if http2writeEndsStream(wr.write) {
		...
		case http2stateOpen:
			...
			// theory, but since our http.Handler is done and
...
func (sc *http2serverConn) processData(f *http2DataFrame) error {
	...
	if f.Length > 0 {
		...
		if len(data) > 0 {
			...
			if err != nil {
				// The http.Handler has closed the request body.
...
func (sc *http2serverConn) upgradeRequest(req *http.Request) {
	...
	// so start the http.Handler directly rather than going
...
// scheduleHandler starts a http.Handler goroutine,
// or schedules one to start as soon as an existing http.Handler finishes.
...
func (sc *http2serverConn) writeHeaders(st *http2stream, headerData *http2writeResHeaders) error {
	...
	if headerData.h != nil {
		...
		// writes out the correct value of keys, before a http.Handler later potentially
		// mutates it.
...
// called from http.Handler goroutines.
// Notes that the http.Handler for the given stream ID read n bytes of its body
...
// request (in http.HandlerDone) and calls on the responseWriter thereafter
...
type http2responseWriterState struct {
	...
	snapHeader    http.Header // snapshot of http.HandlerHeader at WriteHeader time
	trailers      []string    // set in writeChunk
	status        int         // status code passed to WriteHeader
	wroteHeader   bool        // WriteHeader called (explicitly or implicitly). Not necessarily sent to user yet.
	sentHeader    bool        // have we sent the header frame?
	handlerDone   bool        // http.Handler has finished

	sentContentLen int64 // non-zero if http.Handler set a Content-Length header
	wroteBytes     int64
...
//
// This method runs after the http.Handler is done and promotes any Header
...
func (w *http2responseWriter) write(lenData int, dataB []byte, dataS string) (n int, err error) {
	...
	if rws.sentContentLen != 0 && rws.wroteBytes > rws.sentContentLen {
		...
		return 0, errors.New("http2: http.Handler wrote more than declared Content-Length")
	}
...
func (sc *http2serverConn) startPush(msg *http2startPushRequest) {
	...
	// is written. Once the ID is allocated, we start the request http.Handler.
	allocatePromisedID := func() (uint32, error) {
		...
		rw, req, err := sc.newWriterAndRequestNoBody(promised, http2requestParam{
			...
			header:    http2cloneHeader(msg.header), // clone since http.Handler runs concurrently with writing the PUSH_PROMISE
		})
		if err != nil {
...
type Http2Transport struct {
	...
	connPoolOrDef ClientConnPool2 // non-nil version of ConnPool
...
func (t *Http2Transport) AddConn(conn net.Conn, addr string) (used bool, err error) {
	used, err = t.connPool().AddConnIfNeeded(addr, t, conn.(reqtls.Conn))
	return
}

func (t *Http2Transport) connPool() ClientConnPool2 {
...
func (t *Http2Transport) initConnPool() {
	if t.ConnPool != nil {
		...
		t.connPoolOrDef = &clientConnPool2{t: t}
	}
}
...
func (t *Http2Transport) RoundTripOnlyCachedConn(req *http.Request) (*http.Response, error) {
	return t.RoundTripOpt(req, http2RoundTripOpt{OnlyCachedConn: true})
}
...
func (t *Http2Transport) newClientConn(c net.Conn, singleUse bool) (*http2ClientConn, error) {
	...
	if len(t.Settings) > 0 {
		for _, v := range t.Settings {
			initialSettings = append(initialSettings, http2Setting{
				ID:  http2SettingID(v.ID),
				Val: v.Val,
			})
		}
	} else {
		initialSettings = []http2Setting{
			{ID: http2SettingID(http2.SettingEnablePush), Val: 0},
			{ID: http2SettingID(http2.SettingInitialWindowSize), Val: 4 << 20},
		}
		if max := t.maxHeaderListSize(); max != 0 {
			initialSettings = append(initialSettings, http2Setting{ID: http2SettingID(http2.SettingMaxHeaderListSize), Val: max})
		}
	}
	...
	if connFlow < 1 {
		connFlow = http2transportDefaultConnFlow
	}
	cc.fr.WriteWindowUpdate(0, connFlow)
	for _, p := range t.PriorityFrames {
		cc.fr.WritePriority(p.StreamID, http2PriorityParam(p.PriorityParam))
		cc.nextStreamID = p.StreamID + 2
	}
	cc.inflow.init(int32(connFlow) + http2initialWindowSize)
	cc.bw.Flush()
	if cc.werr != nil {
		cc.Close()
		return nil, cc.werr
...
func (rl *http2clientConnReadLoop) processHeaders(f *http2MetaHeadersFrame) error {
	...
	if res == nil {
		// (nil, nil) special case. See http.Handleresponse docs.
		return nil
	}
...
// is the detail.
//
// As a special case, http.Handleresponse may return (nil, nil) to skip the
...
// connection.
func (t *Http2Transport) dialTLSWithContext(ctx context.Context, network, addr string, cfg *tls.Config) (reqtls.Conn, error) {
...
//
// This interface is implemented by *http.ServerConn.
//
...
// http.HandlerPanicRST is the message sent from http.Handler goroutines when
// the http.Handler panics.
...
// for HTTP response headers or trailers from a server http.Handler.
...
func (wr http2FrameWriteRequest) StreamID() uint32 {
	if wr.stream == nil {
		if se, ok := wr.write.(http2StreamError); ok {
			// (*http.ServerConn).resetStream doesn't set
